...
def euler_runge_error(f, x0, y0, x_end, h):
    p = 2
...
def rk4_runge_error(f, x0, y0, x_end, h):
    p = 4
...
def run_session(eq_key, x0, y0, x_end, h, eps, output_dir, session_id):
    ...
    r_euler, _, _ = euler_runge_error(f, x0, y0, x_end, h)
    r_rk4,   _, _ = rk4_runge_error(f, x0, y0, x_end, h)
...
def main():
    ...
    print("Методы: Усовершенств. Эйлер | РК4 | Адамс" )
